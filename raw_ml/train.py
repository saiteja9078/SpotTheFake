...
def load_dataset():
    ...
    real_dir = os.path.join(PROJECT_DIR, "Dataset", "Real")
    screen_dir = os.path.join(PROJECT_DIR, "Dataset", "Screen")
...
def train_and_evaluate(X_train, y_train, X_test, y_test, feature_names):
    ...
    print(f"\n{'─' * 60}")
    print(f"  Training: VotingEnsemble (top 3 by CV F1)")
    print(f"{'─' * 60}")

    # Sort by test F1 and pick top 3
    ...
    # Re-create top 3 with their best params
    all_models_defs = get_models_and_params()
    top3_estimators = []
    for tname in top3_names:
        base_est, _ = all_models_defs[tname]
        best_params = results[tname]["best_params"]
        est = base_est.__class__(**{**base_est.get_params(), **best_params})
        top3_estimators.append((tname, est))
    ...
    vote_test_f1 = f1_score(y_test, y_pred_vote)
    vote_test_prec = precision_score(y_test, y_pred_vote, zero_division=0)
    vote_test_rec = recall_score(y_test, y_pred_vote, zero_division=0)
    vote_test_cm = confusion_matrix(y_test, y_pred_vote)

    cv_acc_vote = cross_val_score(voting_clf, X_train_scaled, y_train, cv=cv, scoring="accuracy")
    cv_f1_vote = cross_val_score(voting_clf, X_train_scaled, y_train, cv=cv, scoring="f1")

    results["VotingEnsemble"] = {
        "cv_accuracy_mean": float(np.mean(cv_acc_vote)),
        "cv_accuracy_std": float(np.std(cv_acc_vote)),
        "cv_f1_mean": float(np.mean(cv_f1_vote)),
        "cv_f1_std": float(np.std(cv_f1_vote)),
        "test_accuracy": float(vote_test_acc),
        "test_f1": float(vote_test_f1),
        "test_precision": float(vote_test_prec),
        "test_recall": float(vote_test_rec),
        "test_confusion_matrix": vote_test_cm.tolist(),
        "best_params": {"members": top3_names},
        "train_time_sec": round(elapsed, 2),
    }

    print(f"  CV Accuracy:  {np.mean(cv_acc_vote):.4f} ± {np.std(cv_acc_vote):.4f}")
    print(f"  CV F1:        {np.mean(cv_f1_vote):.4f} ± {np.std(cv_f1_vote):.4f}")
    print(f"  Test Accuracy: {vote_test_acc:.4f}")
    print(f"  Test F1:       {vote_test_f1:.4f}")
    print(f"  Confusion Matrix: {vote_test_cm.tolist()}")

    if vote_test_f1 > best_overall_test_f1:
        best_overall_test_f1 = vote_test_f1
        best_overall_model = voting_clf
        best_overall_name = "VotingEnsemble"

    # ---------- Stacking Ensemble ----------
    print(f"\n{'─' * 60}")
    print(f"  Training: StackingEnsemble (top 3 + LR meta)")
    print(f"{'─' * 60}")

    top3_estimators_stack = []
    for tname in top3_names:
        base_est, _ = all_models_defs[tname]
        best_params = results[tname]["best_params"]
        est = base_est.__class__(**{**base_est.get_params(), **best_params})
        top3_estimators_stack.append((tname, est))

    stacking_clf = StackingClassifier(
        estimators=top3_estimators_stack,
    ...
    stack_test_acc = accuracy_score(y_test, y_pred_stack)
    stack_test_f1 = f1_score(y_test, y_pred_stack)
    stack_test_prec = precision_score(y_test, y_pred_stack, zero_division=0)
    stack_test_rec = recall_score(y_test, y_pred_stack, zero_division=0)
    stack_test_cm = confusion_matrix(y_test, y_pred_stack)

    cv_acc_stack = cross_val_score(stacking_clf, X_train_scaled, y_train, cv=cv, scoring="accuracy")
    cv_f1_stack = cross_val_score(stacking_clf, X_train_scaled, y_train, cv=cv, scoring="f1")

    results["StackingEnsemble"] = {
        "cv_accuracy_mean": float(np.mean(cv_acc_stack)),
        "cv_accuracy_std": float(np.std(cv_acc_stack)),
        "cv_f1_mean": float(np.mean(cv_f1_stack)),
        "cv_f1_std": float(np.std(cv_f1_stack)),
        "test_accuracy": float(stack_test_acc),
        "test_f1": float(stack_test_f1),
        "test_precision": float(stack_test_prec),
        "test_recall": float(stack_test_rec),
        "test_confusion_matrix": stack_test_cm.tolist(),
        "best_params": {"members": top3_names, "meta": "LogisticRegression"},
        "train_time_sec": round(elapsed, 2),
    }

    print(f"  CV Accuracy:  {np.mean(cv_acc_stack):.4f} ± {np.std(cv_acc_stack):.4f}")
    print(f"  CV F1:        {np.mean(cv_f1_stack):.4f} ± {np.std(cv_f1_stack):.4f}")
    print(f"  Test Accuracy: {stack_test_acc:.4f}")
    print(f"  Test F1:       {stack_test_f1:.4f}")
    print(f"  Confusion Matrix: {stack_test_cm.tolist()}")

    if stack_test_f1 > best_overall_test_f1:
        best_overall_test_f1 = stack_test_f1
        best_overall_model = stacking_clf
        best_overall_name = "StackingEnsemble"
